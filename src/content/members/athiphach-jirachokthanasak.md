---
order: 2
name:
  th: อธิพัชร์ จิรโชคธนะศักดิ์
  en: Athiphach Jirachokthanasak
role:
  th: ผู้ร่วมก่อตั้ง | ผู้ประสานงานชุมชน
  en: Co-Founder | Community Coordinator
photo: ../../assets/members/athiphach-jirachokthanasak.png
bioShort:
  th: >-
    ผู้ร่วมก่อตั้งและผู้ประสานงานชุมชน NavaLab อดีตนายประจำเรือและผู้เชี่ยวชาญด้านปฏิบัติการเดินเรือ
  en: >-
    Co-Founder and Community Coordinator of NavaLab, with a background as a deck officer and
    maritime operations specialist.
bio:
  th: >-
    อธิพัชร์เกิดที่จังหวัดชลบุรี เริ่มต้นเส้นทางในอุตสาหกรรมทางทะเลในตำแหน่งนายประจำเรือที่ Highland
    Maritime ก่อนย้ายมาทำงานด้านปฏิบัติการท่าเรือและตัวแทนเรือกับ A.P. Moller - Maersk และต่อมาเป็น
    ผู้วางแผนด้านความปลอดภัยทางทะเลระดับภูมิภาคที่ VIKING Life-Saving Equipment ปัจจุบันกำลังศึกษาต่อ
    ปริญญาโทด้านเทคโนโลยีทางทะเลและโลจิสติกส์ที่ Tokyo University of Marine Science and Technology
    โดยมีงานวิจัยที่สนใจคือการบริหารจัดการการจราจรทางน้ำและการเลือกเส้นทางเดินเรือในพื้นที่หนาแน่น
    ในฐานะผู้ร่วมก่อตั้ง NavaLab อธิพัชร์ดูแลด้านการประสานงานชุมชนและเชื่อมโยงสมาชิกในกลุ่ม
  en: >-
    Born in Chon Buri, Thailand, Athiphach began his maritime career as a Third Officer at
    Highland Maritime before moving into port operations and agency work with A.P. Moller -
    Maersk, and later regional marine safety planning at VIKING Life-Saving Equipment. He is
    currently pursuing a Master's degree in Maritime Technology and Logistics at Tokyo University
    of Marine Science and Technology, with a research focus on optimizing traffic management and
    route selection in congested sea areas. As Co-Founder of NavaLab, he leads community
    coordination and connects the group's members.
expertise:
  th:
    - ปฏิบัติการเดินเรือ
    - ความปลอดภัยทางทะเล
    - การประสานงานชุมชน
  en:
    - Vessel Operations
    - Maritime Safety
    - Community Coordination
experience:
  - role:
      th: ผู้วางแผนด้านความปลอดภัยทางทะเลระดับภูมิภาค
      en: Regional Marine Fire Service Planner
    organization: VIKING Life-Saving Equipment
    period: Oct 2021 – Sep 2025
  - role:
      th: เจ้าหน้าที่บริหารฝ่ายบก
      en: Landside Executive Agent
    organization: A.P. Moller - Maersk
    period: Nov 2019 – Jul 2021
  - role:
      th: เจ้าหน้าที่ปฏิบัติการ - ตัวแทนเรือ
      en: Operations Executive - Port Agency
    organization: A.P. Moller - Maersk
    period: Aug 2019 – Nov 2019
  - role:
      th: นายประจำเรือ (ต้นเรือสาม)
      en: Third Officer
    organization: Highland Maritime
    period: May 2017 – Feb 2019
education:
  - degree:
      th: ปริญญาตรี
      en: Bachelor's Degree
    field:
      th: วิทยาการเดินเรือ
      en: Nautical Science
    institution:
      th: มหาวิทยาลัยเกษตรศาสตร์
      en: Kasetsart University
  - degree:
      th: ปริญญาโท (กำลังศึกษา)
      en: Master's Degree (In Progress)
    field:
      th: เทคโนโลยีทางทะเลและโลจิสติกส์
      en: Maritime Technology and Logistics
    institution:
      th: Tokyo University of Marine Science and Technology
      en: Tokyo University of Marine Science and Technology
research:
  current:
    - title:
        th: การบริหารจัดการการจราจรทางน้ำและการเลือกเส้นทางในพื้นที่หนาแน่น
        en: Optimization of Traffic Management and Route Selection in Congested Sea Areas
      summary:
        th: >-
          ศึกษาแนวทางบริหารจัดการการจราจรทางน้ำเพื่อเพิ่มประสิทธิภาพและความปลอดภัยในพื้นที่ที่มีความหนาแน่นของเรือสูง
        en: >-
          Studying traffic management approaches to improve efficiency and safety in high-density
          shipping areas.
  completed: []
publications: []
links:
  email: j.athiphach@gmail.com
  linkedin: https://www.linkedin.com/in/athiphach-jirachokthanasak
featured: true
---
